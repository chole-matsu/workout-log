...
- **リセット**：入力中の重量と回数だけを空にする。保存済みの記録とメモには触らない
- **推移グラフ** — 最大重量 / 総挙上量を折れ線で表示（直近20件）
- **履歴一覧** — 過去の記録を新しい順に一覧表示
- **記録の編集・削除** — 履歴の行をタップすると、その日の記録を editable な入力欄に読み込む
- **並び替え**：お気に入り / 追加順 / 名前順 / 更新順 / 色順 の5基準。選択中のチップをもう一度押すと昇順⇄降順が入れ替わる
- **お気に入り配置**：タイルを長押ししてドラッグすると、好きな位置に並べ替えられる
...
| 色順 | 赤→紫（色相順） | 紫→赤 | 昇順 |
| お気に入り | 自分で決めた並び | その逆順 | 昇順 |

- 名前順は `localeCompare(name, 'ja')` を使うため、ひらがな・カタカナ・漢字・英字が自然な順に並びます
- 更新順では、まだ記録がない種目は**向きに関わらず常に末尾**に置かれます
- 色順は色を色相（0〜360度）に変換して並べます。白・黒・灰は色相を持たないため末尾に回ります
- お気に入り順の並びは「種目 id の配列」として保存します。Exercise 自体に順番を持たせないので
  既存データの作り直しが要りません。並びに無い種目（新しく追加したもの）は末尾に付きます
- 選んだ並び順は端末に保存され、アプリを再起動しても維持されます

### ドラッグでの並べ替え（お気に入り順のとき）

`src/components/DraggableGrid.tsx` が担当します。追加のライブラリは使わず、
React Native 標準の `PanResponder` と `Animated` だけで組んでいます。

- タップ・長押し・ドラッグを**タイル1枚につき PanResponder 1つ**で扱います。
  `Pressable` と併用すると、指を置いた時点で `Pressable` がレスポンダを取ってしまい、
  指の動きがドラッグ処理に届きません
- スクロールとの両立は `onPanResponderTerminationRequest` で行います。
  ドラッグ中でなければ `ScrollView` にレスポンダを譲るので、普通にスクロールできます
- **位置の判定はつねに「ドラッグ開始時のマス＋移動量」で行います。** 基準点を並べ替えのたびに
  ずらすと補正が二重に効いて、前方向へ飛びすぎます（実際にこのバグを踏みました）
- タイルの絶対配置に高さが要るため、カードの高さは `cardHeightFor()` で計算した固定値です。
  カードの中身の行数やフォントサイズを変えたら、この関数も合わせて直してください
- お気に入り順**以外**では、実績のある `Pressable` のグリッドをそのまま使います
...
src/components/DraggableGrid.tsx  長押し＋ドラッグで並べ替えられるグリッド
